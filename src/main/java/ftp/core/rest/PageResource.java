...
public class PageResource<T> extends RepresentationModel implements Page<T> {
    ...
    public PageResource(Page<T> page, String pageParam, String sizeParam) {
        ...
        Link link = buildPageLink(pageParam, 0, sizeParam, page.getSize(), IanaLinkRelations.FIRST);
        add(link);

        int indexOfLastPage = page.getTotalPages() - 1;
        link = buildPageLink(pageParam, indexOfLastPage, sizeParam, page.getSize(), IanaLinkRelations.LAST);
        add(link);

        link = buildPageLink(pageParam, page.getNumber(), sizeParam, page.getSize(), IanaLinkRelations.SELF);
        add(link);
    }
     ...
     */
    private Link buildPageLink(String pageParam, int page, String sizeParam, int size, LinkRelation rel) {
