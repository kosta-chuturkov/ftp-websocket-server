 ...
 * for audit purposes <li> <li>When a user logs out, only his current session is invalidated, and
 * not all of his sessions</li> </ul> <p> This is inspired by: <ul> <li><a href=
 * "http://jaspan.com/improved_persistent_login_cookie_best_practice">Improved Persistent Login
 * Cookie Best Practice</a></li> <li><a href="https://github.com/blog/1661-modeling-your-app-s-user-session">
 * Github's "Modeling your App's User Session"</a></li> </ul> <p> The main algorithm comes from
 * Spring Security's PersistentTokenBasedRememberMeServices, but this class couldn't be cleanly
 * extended.
 */
@Service
public class CustomPersistentRememberMeServices extends AbstractRememberMeServices {
  ...
  protected UserDetails processAutoLoginCookie(final String[] cookieTokens,
    ...
    this.log.debug("Refreshing persistent login token for user '{}', series '{}'", login,
        token.getSeries());
  ...
  /**
   * When logout occurs, only invalidate the current token, and not all user sessions. <p> The
   * standard Spring Security implementations are too basic: they invalidate all tokens for the
