...
UserServiceTest extends AbstractTest {
    ...
    public void testGetUserByNickLike() {
        ...
        usersContainingAdmString.containsAll(Lists.newArrayList(badmin, ladadmin3, User.getCurrent().getNickName()));
    }
    ...
    public void testAddFileToUser() {
        ...
        //when
        this.userService.addFileToUser(savedFile.getId(), User.getCurrent().getId());

        //then
        UploadedFilesProjection uploadedFilesProjection = this.userService.findUploadedFilesByUserId(User.getCurrent().getId());
